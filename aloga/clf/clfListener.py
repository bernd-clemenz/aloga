# Generated from clf.g4 by ANTLR 4.8
from antlr4 import *
if __name__ is not None and "." in __name__:
    from .clfParser import clfParser
else:
    from clfParser import clfParser

# This class defines a complete listener for a parse tree produced by clfParser.
class clfListener(ParseTreeListener):

    # Enter a parse tree produced by clfParser#log.
    def enterLog(self, ctx:clfParser.LogContext):
        pass

    # Exit a parse tree produced by clfParser#log.
    def exitLog(self, ctx:clfParser.LogContext):
        pass


    # Enter a parse tree produced by clfParser#line.
    def enterLine(self, ctx:clfParser.LineContext):
        pass

    # Exit a parse tree produced by clfParser#line.
    def exitLine(self, ctx:clfParser.LineContext):
        pass


    # Enter a parse tree produced by clfParser#host.
    def enterHost(self, ctx:clfParser.HostContext):
        pass

    # Exit a parse tree produced by clfParser#host.
    def exitHost(self, ctx:clfParser.HostContext):
        pass


    # Enter a parse tree produced by clfParser#logname.
    def enterLogname(self, ctx:clfParser.LognameContext):
        pass

    # Exit a parse tree produced by clfParser#logname.
    def exitLogname(self, ctx:clfParser.LognameContext):
        pass


    # Enter a parse tree produced by clfParser#username.
    def enterUsername(self, ctx:clfParser.UsernameContext):
        pass

    # Exit a parse tree produced by clfParser#username.
    def exitUsername(self, ctx:clfParser.UsernameContext):
        pass


    # Enter a parse tree produced by clfParser#datetimetz.
    def enterDatetimetz(self, ctx:clfParser.DatetimetzContext):
        pass

    # Exit a parse tree produced by clfParser#datetimetz.
    def exitDatetimetz(self, ctx:clfParser.DatetimetzContext):
        pass


    # Enter a parse tree produced by clfParser#referer.
    def enterReferer(self, ctx:clfParser.RefererContext):
        pass

    # Exit a parse tree produced by clfParser#referer.
    def exitReferer(self, ctx:clfParser.RefererContext):
        pass


    # Enter a parse tree produced by clfParser#request.
    def enterRequest(self, ctx:clfParser.RequestContext):
        pass

    # Exit a parse tree produced by clfParser#request.
    def exitRequest(self, ctx:clfParser.RequestContext):
        pass


    # Enter a parse tree produced by clfParser#useragent.
    def enterUseragent(self, ctx:clfParser.UseragentContext):
        pass

    # Exit a parse tree produced by clfParser#useragent.
    def exitUseragent(self, ctx:clfParser.UseragentContext):
        pass


    # Enter a parse tree produced by clfParser#statuscode.
    def enterStatuscode(self, ctx:clfParser.StatuscodeContext):
        pass

    # Exit a parse tree produced by clfParser#statuscode.
    def exitStatuscode(self, ctx:clfParser.StatuscodeContext):
        pass


    # Enter a parse tree produced by clfParser#bytes.
    def enterBytes(self, ctx:clfParser.BytesContext):
        pass

    # Exit a parse tree produced by clfParser#bytes.
    def exitBytes(self, ctx:clfParser.BytesContext):
        pass



del clfParser